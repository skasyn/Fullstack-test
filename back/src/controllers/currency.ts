...
export const getAllCurrency = async (req: Request, res: Response) => {
  const currencies =
    getFromCache('currencies') ?? addToCache('currencies', (await getCurrencies()).data.results);
...
export const getConversionRate = async (
  req: Request<unknown, unknown, unknown, GetConversionRateInterface>,
  res: Response,
) => {
  const { query } = req;
  ...
  const conversionRate =
    getFromCache(query.conversion) ??
    addToCache(query.conversion, (await getConversion(query.conversion)).data);
