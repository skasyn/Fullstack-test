...
const buildCurrenciesQuery = (newCurr: string, isFrom: boolean, oldFromCurr: string, oldToCurr: string) => {
  if (isFrom) {
...
export const ConvertGrid = ({ currencies }: GridProps) => {
  ...
  const onCurrencyChange = async (newCurr: string, isFrom: boolean) => {
    const { currenciesQuery, from, to } = buildCurrenciesQuery(newCurr, isFrom, state.fromCurrency, state.toCurrency);
    const res = await fetch(`http://localhost:8000/convert?conversion=${currenciesQuery}`);
    const rate = (await res.json())[currenciesQuery];

    dispatch({ type: "changeCurrency", payload: { fromCurrency: from, toCurrency: to, newRate: rate }})
  };
  ...
  return (
    <Grid
      ...
      >
        <GridItem>
          <NumberInput>
            <NumberInputField placeholder="From" onChange={(e) => dispatch({ type: "changeValues", payload: { from: Number(e.target.value)}})}/>
          </NumberInput>
        </GridItem>
        <GridItem>
        <Select placeholder="USD" onChange={(e) => onCurrencyChange(e.target.value, true)}>
          { Object.keys(currencies).map((e) => <option key={`${e}_from`} value={e}>{e}</option>)}
          </Select>
        </GridItem>
        <GridItem>
          <Result />
        ...
        { Object.keys(currencies).map((e) => <option key={`${e}_to`} value={e}>{e}</option>)}
          </Select>
        </GridItem>
      </Grid>
  )
